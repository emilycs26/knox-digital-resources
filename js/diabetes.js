...
function renderProgressBar() {
  ...
  modules.forEach((mod, mi) => {
    ...
    markers.push({ type:'flag', pct:Math.min(base+spacing,100), done:modDone, id:`flag-${mod}`, label:modLabels[mi], mod });
  });
  ...
  const markersHTML = markers.map(m => m.type === 'dot'
    ? `<div class="progress-marker" style="left:${m.pct}%" id="${m.id}"><span class="p-dot ${m.done?'done':''}"></span></div>`
    : `<div class="progress-marker" style="left:${m.pct===100?'calc(100% - 4px)':m.pct+'%'}" id="${m.id}"><span class="p-flag ${m.done?'done':''}">⚑</span></div>`
  ).join('');
...
function markSubTabDone(tabName) {
  ...
  const allDone = progressState.subTabs.diabetes.every(s => progressState.completedSubs.diabetes.has(s));
...
function triggerStarBurst(flagId) {
  ...
  ['⭐','✨','🌟','💫','⭐','✨','🌟','💫'].forEach((s,i) => {
    const angle = (i/8)*2*Math.PI, dist = 60+Math.random()*40;
...
const typeDetails = {
  t1: {
    ...
    keyFacts:['Accounts for approximately 5–10% of all cases','Can develop at any age (commonly appears in children and young adults)','Symptoms typically come on quickly, over days or weeks','It is not caused by diet or lifestyle','There is currently no cure — insulin therapy is required for life'],
    ...
    extra:{ type:'mythbust', title:'Myth Busting', subtitle:'Click on each card to reveal whether each statement is true or false.',
      myths:[
  ...
  t2: {
    ...
    keyFacts:['Most common form of diabetes — 90–95% of all cases',"The body still produces insulin but cells don't respond to it effectively → insulin resistance","The pancreas tries to compensate by making more insulin, but eventually can't keep up",'Type 2 typically develops gradually (over months or years)','Many people have no symptoms at first'],
    ...
    extra:{ type:'checklist', title:'Risk Factor Checklist', subtitle:'Select any risk factors that apply to you:',
      riskFactors:[
  ...
  pre: {
    ...
    table:{ headers:['','Normal','Prediabetes','Diabetes'], rows:[['A1C','Below 5.7%','5.7% – 6.4%','6.5% or above'],['Fasting Blood Sugar','Below 100 mg/dL','100 – 125 mg/dL','126 mg/dL or above']] },
    whyItMatters:['People with prediabetes have a yearly conversion rate to Type 2 of approximately 5–10%','Prediabetes also increases risk for cardiovascular disease','Lifestyle changes can reduce the risk of progressing to Type 2 by 40–70%'],
    whatYouCanDo:['Eat a balanced diet with more vegetables, whole grains, and lean proteins','Aim for at least 150 minutes of moderate physical activity per week','Lose 5–7% of body weight if overweight','Talk to your doctor about screening — the ADA recommends screening starting at age 35']
  }
};
...
function buildTypePanel(key) {
  ...
  if (key === 'pre') {
    const tableRows = d.table.rows.map(row => `<tr>${row.map((c,i) => i===0?`<td style="font-weight:600">${c}</td>`:`<td>${c}</td>`).join('')}</tr>`).join('');
    return `<div class="type-panel" style="background:${bg};border-color:${bc}">
      ...
      <div class="type-panel-section" style="border-color:${bc}">${d.intro.split('\n\n').map(p=>`<p style="margin-bottom:10px;">${p}</p>`).join('')}</div>
      <div class="type-panel-section" style="border-color:${bc}"><table class="pre-table"><thead><tr>${d.table.headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${tableRows}</tbody></table></div>
      <div class="type-panel-section" style="border-color:${bc}">
        <p style="font-weight:600;margin-bottom:6px;">Why It Matters:</p><ul style="padding-left:18px;margin-bottom:14px;">${d.whyItMatters.map(x=>`<li>${x}</li>`).join('')}</ul>
        <p style="font-weight:600;margin-bottom:6px;">What You Can Do:</p><ul style="padding-left:18px;">${d.whatYouCanDo.map(x=>`<li>${x}</li>`).join('')}</ul>
      </div></div>`;
  }
  const keyFactsHTML = d.keyFacts.map(f=>`<li>${f}</li>`).join('');
  const sympTreatHTML = d.symptoms ? `<div class="type-panel-cols" style="border-color:${bc}">
    <div class="type-panel-col" style="border-color:${bc}"><p style="font-weight:600;margin-bottom:8px;">Common Symptoms:</p><ul>${d.symptoms.map(s=>`<li>${s}</li>`).join('')}</ul></div>
    <div class="type-panel-col"><p style="font-weight:600;margin-bottom:8px;">Treatment:</p><p>${d.treatment}</p></div></div>` : '';
  let extraHTML = '';
  if (d.extra?.type === 'mythbust') {
    extraHTML = `<div class="type-panel-section" style="border-color:${bc}">
      ...
      ${d.extra.myths.map((m,i)=>`<div class="myth-card" data-myth="${i}" style="background:${ac}">
        <span class="myth-front">${m.statement}</span>
        <div class="myth-back" style="display:none"><span class="myth-verdict ${m.verdict==='TRUE'?'verdict-true':'verdict-false'}">${m.verdict}</span><span>${m.explanation}</span></div>
      </div>`).join('')}</div>`;
  }
  if (d.extra?.type === 'checklist') {
    extraHTML = `<div class="type-panel-section" style="border-color:${bc}">
      ...
      <div id="riskCheckList">${d.extra.riskFactors.map(r=>`<div class="risk-check-row"><label class="risk-check-label" for="${r.id}">${r.label}</label><input type="checkbox" class="risk-checkbox" id="${r.id}"></div>`).join('')}</div>
  ...
  return `<div class="type-panel" style="background:${bg};border-color:${bc}">
    <button class="type-panel-close" style="color:${ac}" data-close="true">✕</button>
    <div class="type-panel-section" style="border-color:${bc}"><p style="font-weight:600;margin-bottom:6px;">Key Facts:</p><ul style="padding-left:18px;">${keyFactsHTML}</ul></div>
    ${sympTreatHTML}${extraHTML}</div>`;
}
...
window.evaluateRisk = function(borderColor) {
  ...
  msg.innerHTML = text; msg.style.display = 'block'; msg.style.borderColor = borderColor;
};
...
function renderIntroduction() {
  ...
  document.getElementById('tab-introduction').innerHTML = `
    ...
    <div class="next-btn-row"><button class="next-btn" id="introNextBtn">Go to Diet &rarr;</button></div>`;
  ...
  function openTypePanel(key) {
    ...
    if (activeType === key) { container.innerHTML=''; grid.style.display=''; center.style.display=''; activeType=null; return; }
    activeType = key;
    grid.style.display='none'; center.style.display='none';
    container.innerHTML = buildTypePanel(key);
    container.scrollIntoView({ behavior:'smooth', block:'nearest' });
    container.querySelector('[data-close]').addEventListener('click', () => { container.innerHTML=''; grid.style.display=''; center.style.display=''; activeType=null; });
...
// ── PLATE DATA ────────────────────────────────────
// TO SWAP IN A NOUN PROJECT ICON: replace each <g class="plate-icon"> with:
//   <image href="path/to/icon.svg" x="..." y="..." width="56" height="56"
//     style="filter:brightness(0) invert(1);pointer-events:none;" />
const plateSegData = {
  veg: {
    label:'Nonstarchy Vegetables', pct:'50%', bg:'#e8f5e0', border:'#b8dca0', textColor:'#336600',
    ...
    // Veg icon centered in bottom half (a1:90, a2:270) — swap <g> for <image href="..." x="100" y="183" width="56" height="56" .../>
  ...
  carb: {
    label:'Carbs', pct:'25%', bg:'#fff3e0', border:'#ffcc80', textColor:'#e65100',
    ...
    // Carb icon centered in top-left quarter (a1:270, a2:360) — swap <g> for <image href="..." x="63" y="63" width="56" height="56" .../>
  ...
  protein: {
    label:'Protein', pct:'25%', bg:'#fce4e4', border:'#ef9a9a', textColor:'#b71c1c',
    detail:'Aim for lean options like chicken or turkey (no skin), fish, tofu, or eggs.',
    serving:'3 oz cooked',
    // Protein icon centered in top-right quarter (a1:0, a2:90) — swap <g> for <image href="..." x="183" y="63" width="56" height="56" .../>
...
// ── SIDE ITEM DATA ────────────────────────────────
// TO SWAP ICON: replace svgIcon with <img src="path/to/icon.svg" width="44" height="44" style="filter:brightness(0) invert(1)">
const sideItemData = [
  { id:'beverages', circleColor:'#4dd0e1', hoverColor:'#00acc1', cardBg:'#e0f7fa', cardBorder:'#80deea', cardTextColor:'#006064',
    name:'Beverages', title:'Beverages', body:'Options include:', bullets:['Water','Plain tea','Coffee','Diet soda','Diet tea'], serving:null,
  ...
  { id:'fruit', circleColor:'#f06292', hoverColor:'#e91e8c', cardBg:'#fce4ec', cardBorder:'#f48fb1', cardTextColor:'#880e4f',
    name:'Fruit', title:'Fruit', body:'A great source of vitamins, minerals, and fiber. Served on the side to help manage your total carb intake.', bullets:null, serving:'½ cup fresh',
    svgIcon:`<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" style="pointer-events:none">
  ...
  { id:'dairy', circleColor:'#9575cd', hoverColor:'#7b1fa2', cardBg:'#f3e5f5', cardBorder:'#ce93d8', cardTextColor:'#4a148c',
    name:'Dairy', title:'Dairy', body:'Choose low-fat or fat-free options.', bullets:['Skim milk','1% milk','Plain yogurt','Plain Greek yogurt','Soy milk'], serving:'1 cup',
    svgIcon:`<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" style="pointer-events:none">
...
function renderDiet() {
  document.getElementById('tab-diet').innerHTML = `
    ...
    <div class="next-btn-row"><button class="next-btn" id="dietNextBtn">Go to Medication &nbsp;›</button></div>`;
  ...
  document.querySelectorAll('.plate-seg').forEach(el => {
    el.addEventListener('mouseenter', () => { allCardIds.forEach(id => { document.getElementById(id).style.display='none'; }); const t=document.getElementById(segCardMap[el.dataset.seg]); if(t) t.style.display='block'; });
    el.addEventListener('mouseleave', () => { allCardIds.forEach(id => { document.getElementById(id).style.display='none'; }); });
  });
...
const msData = {
  ...
  meal: {
    title:'4. Meal Planning', bg:'#f3e5f5',
    render: () => `
  ...
  const paths = slices.map(s => {
    ...
    const textEls = lines.map((l,i) =>
      `<text x="${lp.x.toFixed(1)}" y="${(lp.y+(i-(lines.length-1)/2)*18).toFixed(1)}"
        ...
        style="pointer-events:none">${l}</text>`).join('');
    return `<path class="ms-slice" data-ms="${s.id}" d="${arc(cx,cy,r,s.a1,s.a2)} L${cx} ${cy}Z"
      fill="${s.color}" stroke="#fff" stroke-width="5" style="cursor:pointer;transition:fill 0.18s;"
      onmouseenter="this.setAttribute('fill','${s.hov}')"
      onmouseleave="this.setAttribute('fill','${s.color}')"/>
    ${textEls}`;
  }).join('');
